...
class StrategyEditor(tk.Frame):
    def __init__(self, root: "Root", binance: BinanceClient, *args, **kwargs):
        ...
        self._base_params = [
            ...
            {"code_name": "parameters", "widget": tk.Button, "data_type": float, "text": "Parameters",
             "bg": BG_COLOR2, "command": self._show_popup, "header": "", "width": 10},
            {"code_name": "activation", "widget": tk.Button, "data_type": float, "text": "OFF",
             "bg": "darkred", "command": self._switch_strategy, "header": "", "width" : 6},
            {"code_name": "delete", "widget": tk.Button, "data_type": float, "text": "X",
             "bg": "darkred", "command": self._delete_row, "header": "", "width": 5},
    ...
    def _add_strategy_row(self):
        ...
        for col, base_param in enumerate(self._base_params):
            ...
            if base_param['widget'] == tk.Button:
                self.body_widgets[code_name][b_index].grid(row=b_index, column=col, padx=2)
            else:
                self.body_widgets[code_name][b_index].grid(row=b_index, column=col, padx=11)
