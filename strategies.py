...
class Strategy:
    def __init__(self, client: typing.Union["BinanceClient"], contract: Contract, exchange:str, timeframe: str, balance_pct: float, take_profit: float, stop_loss: float, strat_name):
        ...
        self.candles: typng.list[Candle]  = []
        self.trades: typign.List[Trade] = []
        self.logs = []
    ...
    def parse_trades(self, price: float, size: float, timestamp: int) -> str:
        ...
        if timestamp_diff >= 2000:
            logger.warning("%s %s: %s milliseconds of difference between the current time and the trade time.", self.exchange, self.contract.symbol, timestamp_diff)
        ...
        if timestamp < last_candle.timestamp + self.tf_equiv:
            ...
            missing_candles = (timestamp - last_candle.timestamp) / self.tf_equiv - 1

            logger.info("%s missing %s candle for %s %s (%s %s)", self.exchange, missing_candles, self.contract.symbol, self.tf, timestamp, last_candle.timestamp)
    ...
    def _open_position(self, signal_result: int):
        ...
        if order_status is not None:
            ...
            new_trade = Trade({"time": int(time.time() * 1000), "entry_price": avg_fill_price, "contract": self.contract, "strategy": self.strat_name, "side": position_side, 
